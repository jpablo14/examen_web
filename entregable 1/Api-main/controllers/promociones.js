...
module.exports.actualizar = app.patch('/', (request, response) => {
  ...
  connection.query(sql, values, (error, results) => {
    if (error) throw error;
    response.send(`Promocion con id ${id_promocion} actualizado correctamente`);
  });
});
...
module.exports.agregar = app.post('/', (request, response) => {
  ...
  connection.query(sql, values, (error, results) => {
    if (error) throw error;
    response.status(200).send(`Promocion registrado correctamente con id ${results.insertId}`);
  });
});
...
module.exports.eliminar = app.delete('/:id', (request, response) => {
  ...
  connection.query(sql, id_producto, (error, results) => {
    ...
    if (results.affectedRows > 0) {
      response.status(200).send(`Promocion con id ${id_promocion} eliminado correctamente`);
    } else {
      response.status(404).send(`Promocion con id ${id_promocion} no encontrado`);
    }
  });
});
